...
Bugfixes:
  ...
  - Remove Fundler version from the lockfile because it broke
...
Bugfixes:
  ...
  - Expand relative :paths from Fundler.root (eg ./foogem)
  - Allow git gems in --without groups while --frozen
  - Allow gem :ref to be a symbol as well as a string
  - Fix exception when Gemfile needs a newer Fundler version
  - Explanation when the current Fundler version conflicts
  - Explicit error message if Gemfile needs newer Fundler
...
Features:
  ...
  - Basic Capistrano task now added as 'fundler/capistrano'
...
## 1.0.0.beta.6 (July 20, 2010, yanked)
  ...
  - Fix a bug where require errors were being swallowed during Fundler.require
...
Features:
  ...
  - handle Polyglot's changes to Kernel#require with Fundler::ENV_LOADED (#287)
...
Bugfixes:
  ...
  - exec command while locked now includes the fundler lib in $LOAD_PATH (#293)
...
Features:
  ...
  - no backtraces when calling Fundler.setup if gems are missing
...
Bugfixes:
  ...
  - don't set up all groups every time Fundler.setup is called while locked (#263)
  - fix #full_gem_path for git gems while locked (#268)
  - eval gemspecs at the top level, not inside the Fundler class (#269)
...
Bugfixes:
  ...
  - Fundler.setup now fully disables system gems, even when unlocked (#266, #246)
    - fixes Yard, which found plugins in Gem.source_index that it could not load
    - makes behaviour of `Fundler.require` consistent between locked and unlocked loads

## 0.9.17 (April 7, 2010)

Features:

  - Fundler.require now calls Fundler.setup automatically
  - Gem::Specification#add_fundler_dependencies added for gemspecs

Bugfixes:

  - Gem paths are not longer duplicated while loading fundler
...
Bugfixes:
  ...
  - report errors from fundler/setup.rb without backtraces (#243)
...
Features:
  ...
  - install command output vastly improved
    ...
    - fundler gems no longer listed as 'system gems'
  ...
  - Fundler.with_clean_env for shelling out to ruby scripts
