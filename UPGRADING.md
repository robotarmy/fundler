## Fundler 0.9 to 1.0 and above

Upgrading from Fundler 0.9 to 1.0 is relatively painless. The
Gemfile API is the same, so your old Gemfiles should continue
to work.

The "env" file that 0.9 created at `.fundle/environment.rb` has been
removed. As a side effect of this, Passenger will only find your
fundled gems if you install with `fundle install --deployment`.
Alternatively, you can tell Passenger where you gems are installed,
[something like this](http://bit.ly/passenger-gem-home).

The `fundle lock` command is no longer needed, as the
Gemfile.lock file is now automatically generated by `fundle install`.
If you have not yet done so, add your Gemfile.lock to source control
and check it in.

Running `fundle install` no longer updates the versions of your gems.
If you need to update just one gem, run `fundle update GEMNAME`. To
update all gems to the newest versions possible, run `fundle update`.
...
3. `clear_sources`: Fundler now defaults to an empty source
   ...
   via source "http://gemcutter.org". If you use fundle init, this
   source will be automatically added for you in the generated
   Gemfile
4. `fundle_path`: You can specify this setting when installing
   via `fundle install /path/to/fundle`. Fundler will remember
...
5. `bin_path`: Fundler no longer generates binaries in the root
   of your app. You should use `fundle exec` to execute binaries
...
2. `require 'vendor/gems/environment'`: In unlocked
   ...
   path. In locked, mode, it becomes `require '.fundle/environment'`
