...
module Fundler
  class CLI < Thor
    ...
    def initialize(*)
      ...
      Fundler.ui = UI::Shell.new(the_shell)
      Fundler.ui.debug! if options["verbose"]
      Gem::DefaultUserInteraction.ui = UI::RGProxy.new(Fundler.ui)
    end
    ...
    def init
      ...
      if File.exist?("Gemfile")
        Fundler.ui.error "Gemfile already exists at #{Dir.pwd}/Gemfile"
        exit 1
      end
      ...
      if opts[:gemspec]
        ...
        unless File.exist?(gemspec)
          Fundler.ui.error "Gem specification #{gemspec} doesn't exist"
          exit 1
        end
    ...
    long_desc <<-D
      ...
      all gems are found, Fundler prints a success message and exits with a status of 0.
      If not, the first missing gem is listed and Fundler exits status 1.
    D
    ...
    def check
      ...
      if not_installed.any?
        ...
        Fundler.load.lock
        Fundler.ui.info "The Gemfile's dependencies are satisfied"
      end
    end
    ...
    def install(path = nil)
      ...
      ENV['RB_USER_INSTALL'] = '1' if Fundler::FREEBSD

      # Just disable color in deployment mode
      Fundler.ui.shell = Thor::Shell::Basic.new if opts[:deployment]

      if opts[:production]
        opts[:deployment] = true
        Fundler.ui.warn "The --production option is deprecated, and will be removed in " \
                        "the final release of Fundler 1.0. Please use --deployment instead."
      end

      if (path || opts[:path] || opts[:deployment]) && opts[:system]
        Fundler.ui.error "You have specified both a path to install your gems to, \n" \
      ...
      if opts[:deployment] || opts[:frozen]
        unless Fundler.default_lockfile.exist?
        ...
        if Fundler.root.join("vendor/cache").exist?
          opts[:local] = true
        end

        Fundler.settings[:frozen] = '1'
      end
      ...
      Fundler.settings[:disable_shared_gems] = '1' if Fundler.settings[:path]
      Fundler.settings.without = opts[:without] unless opts[:without].empty?
      Fundler.ui.be_quiet! if opts[:quiet]

      Installer.install(Fundler.root, Fundler.definition, opts)
      ...
      if opts[:local]
        Fundler.ui.warn "Some gems seem to be missing from your vendor/cache directory."
      end

      if Fundler.definition.no_sources?
        Fundler.ui.warn "Your Gemfile doesn't have any sources. You can add one with a line like 'source :rubygems'"
      end
      raise e
    end
    ...
    def update(*gems)
      ...
      if gems.empty? && sources.empty?
        ...
        Fundler.definition(true)
      else
        Fundler.definition(:gems => gems, :sources => sources)
      end
    ...
    def cache
      Fundler.definition.resolve_with_cache!
      Fundler.load.cache
      Fundler.settings[:no_prune] = true if options[:no_prune]
      Fundler.load.lock
    rescue GemNotFound => e
    ...
    def exec(*)
      ...
      begin
        ...
        Fundler.ui.error "fundler: not executable: #{ARGV.first}"
        exit 126
      rescue Errno::ENOENT
    ...
    def config(name = nil, *args)
      ...
      unless name
        Fundler.ui.confirm "Settings are listed in order of priority. The top value will be used.\n"

        Fundler.settings.all.each do |setting|
          Fundler.ui.confirm "#{setting}"
          with_padding do
            Fundler.settings.pretty_values_for(setting).each do |line|
              Fundler.ui.info line
            end
          end
          Fundler.ui.confirm ""
        end
      ...
      if values.empty?
        Fundler.ui.confirm "Settings for `#{name}` in order of priority. The top value will be used"
        with_padding do
          Fundler.settings.pretty_values_for(name).each { |line| Fundler.ui.info line }
        end
      else
        locations = Fundler.settings.locations(name)

        if local = locations[:local]
          Fundler.ui.info "Your application has set #{name} to #{local.inspect}. This will override the " \
            "system value you are currently setting"
        end

        if global = locations[:global]
          Fundler.ui.info "You are replacing the current system value of #{name}, which is currently #{global}"
        end

        if env = locations[:env]
          Fundler.ui.info "You have set a fundler environment variable for #{env}. This will take precedence " \
            "over the system value you are setting"
        end

        Fundler.settings.set_global(name, values.join(" "))
      end
    end
    ...
    desc "version", "Prints the fundler's version information"
    def version
      Fundler.ui.info "Fundler version #{Fundler::VERSION}"
    end
    ...
    def viz
      ...
      graph = Graph.new( Fundler.load )

      begin
        graph.viz(output_file, options[:version], options[:requirements])
        Fundler.ui.info output_file
      rescue LoadError => e
        Fundler.ui.error e.inspect
        Fundler.ui.warn "Make sure you have the graphviz ruby gem. You can install it with:"
        Fundler.ui.warn "`gem install ruby-graphviz`"
      rescue StandardError => e
        if e.message =~ /GraphViz not installed or dot not in PATH/
          Fundler.ui.error e.message
          Fundler.ui.warn "The ruby graphviz gem requires GraphViz to be installed"
        else
          raise
        end
      end
    ...
    def gem(name)
      ...
      Fundler.ui.info "Initializating git repo in #{target}"
