...
=end
    ...
    def initialize(lockfile, dependencies, sources, unlock)
      ...
      if lockfile && File.exists?(lockfile)
        @lockfile_contents = Fundler.read_file(lockfile)
    ...
    def specs
      @specs ||= begin
        ...
        unless specs["fundler"].any?
          local = Fundler.settings[:frozen] ? rubygems_index : index
          fundler = local.search(Gem::Dependency.new('fundler', VERSION)).last
          specs["fundler"] = fundler if fundler
        end
    ...
    def requested_specs
      @requested_specs ||= begin
        groups = self.groups - Fundler.settings.without
