require 'fundler/dependency'

module Fundler
  class Dsl
    def self.evaluate(gemfile, lockfile, unlock)
      builder = new
      builder.instance_eval(Fundler.read_file(gemfile.to_s), gemfile.to_s, 1)
      builder.to_definition(lockfile, unlock)
    end

    VALID_PLATFORMS = Fundler::Dependency::PLATFORM_MAP.keys.freeze
    ...
    def gemspec(opts = nil)
      ...
      path              = File.expand_path(path, Fundler.default_gemfile.dirname)
    ...
    def git(uri, options = {}, source_options = {}, &blk)
      unless block_given?
        msg = "You can no longer specify a git source by itself. Instead, \n" \
              ...
              "fundler should find in the git source by passing a block to \n" \
              "the git method, like: \n\n" \
    ...
    def self.deprecate(name, replacement = nil)
      define_method(name) do |*|
        ...
        message << "\nSee the README for more information on upgrading from Fundler 0.8."
