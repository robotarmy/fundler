...
module Fundler
  class GemHelper
    ...
    def initialize(base, name = nil)
      Fundler.ui = UI::Shell.new(Thor::Base.shell.new)
      @base = base
      gemspecs = name ? [File.join(base, "#{name}.gemspec")] : Dir[File.join(base, "*.gemspec")]
      raise "Unable to determine name from existing gemspec. Use :name => 'gemname' in #install_tasks to manually set it." unless gemspecs.size == 1
      @spec_path = gemspecs.first
      @gemspec = Fundler.load_gemspec(@spec_path)
    end
    ...
    def build_gem
      ...
      sh("gem build #{spec_path}") { |out, code|
        ...
        Fundler.ui.confirm "#{name} #{version} built to pkg/#{file_name}"
      }
    ...
    def install_gem
      ...
      Fundler.ui.confirm "#{name} (#{version}) installed"
    end
    ...
    def rubygem_push(path)
      ...
      Fundler.ui.confirm "Pushed #{name} #{version} to rubygems.org"
    end
    ...
    def git_push
      ...
      Fundler.ui.confirm "Pushed git commits and tags"
    end
    ...
    def tag_version
      ...
      Fundler.ui.confirm "Tagged #{version_tag}"
      yield if block_given?
    rescue
      Fundler.ui.error "Untagged #{version_tag} due to error"
