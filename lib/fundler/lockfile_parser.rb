...
module Fundler
  ...
  private
    ...
    TYPES = {
      "GIT"  => Fundler::Source::Git,
      "GEM"  => Fundler::Source::Rubygems,
      "PATH" => Fundler::Source::Path
    }
    ...
    def parse_dependency(line)
      if line =~ %r{^ {2}#{NAME_VERSION}(!)?$}
        ...
        dep = Fundler::Dependency.new(name, version)

        if pinned && dep.name != 'fundler'
          ...
          if version && version.size == 1 && version.first =~ /^\s*= (.+)\s*$/ && dep.source.is_a?(Fundler::Source::Path)
