...
module Fundler
  class Resolver
    ALL = Fundler::Dependency::PLATFORM_MAP.values.uniq.freeze
    ...
    def resolve(reqs, activated)
      ...
      if existing || current.name == 'fundler'
        # Force the current
        if current.name == 'fundler' && !existing
          existing = search(DepProxy.new(Gem::Dependency.new('fundler', VERSION), Gem::Platform::RUBY)).first
          raise GemNotFound, %Q{Fundler could not find gem "fundler" (#{VERSION})} unless existing
          existing.required_by << existing
          activated['fundler'] = existing
        end
        ...
        if matching_versions.empty?
          if current.required_by.empty?
            if base = @base[current.name] and !base.empty?
              ...
              if @index.sources.include?(Fundler::Source::Rubygems)
            ...
            if @missing_gems[current] >= 5
              message =  "Fundler could not find find gem #{current.required_by.last},"
    ...
    def error_message
      output = errors.inject("") do |o, (conflict, (origin, requirement))|
        ...
        if origin
          ...
          o << %{Fundler could not find compatible versions for gem "#{origin.name}":\n}
          o << "  In Gemfile:\n"

          o << gem_message(requirement)

          # If the origin is "fundler", the conflict is us
          if origin.name == "fundler"
            o << "  Current Fundler version:\n"
            newer_fundler_required = requirement.requirement > Gem::Requirement.new(origin.version)
        ...
        # the specified sources
          ...
          if locked = @base[conflict].first
            o << "Fundler could not find compatible versions for gem #{conflict.inspect}:\n"
            o << "  In snapshot (Gemfile.lock):\n"
            o << "    #{clean_req(locked)}\n\n"
