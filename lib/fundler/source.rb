  ...
  module Source
    ...
    class Rubygems
      ...
      def initialize(options = {})
        ...
        @caches = [ Fundler.app_cache ] + Gem.path.map { |p| File.expand_path("#{p}/cache") }
        @spec_fetch_map = {}
      end
      ...
      def install(spec)
        ...
        if installed_specs[spec].any?
          Fundler.ui.info "Using #{spec.name} (#{spec.version}) "
          return
        end

        Fundler.ui.info "Installing #{spec.name} (#{spec.version}) "

        install_path = Fundler.requires_sudo? ? Fundler.tmp : Gem.dir
        ...
        if Fundler.requires_sudo?
          ...
          sudo "cp -R #{Fundler.tmp}/gems/#{spec.full_name} #{Gem.dir}/gems/"
          sudo "cp -R #{Fundler.tmp}/specifications/#{spec.full_name}.gemspec #{Gem.dir}/specifications/"
          spec.executables.each do |exe|
            sudo "mkdir -p #{Gem.bindir}"
            sudo "cp -R #{Fundler.tmp}/bin/#{exe} #{Gem.bindir}"
          end
        end
      ...
      def cache(spec)
        ...
        return if File.dirname(cached_path) == Fundler.app_cache.to_s
        Fundler.ui.info "  * #{File.basename(cached_path)}"
        FileUtils.cp(cached_path, Fundler.app_cache)
      end
    ...
    private
      ...
      def installed_specs
        @installed_specs ||= begin
          ...
          Gem.source_index.to_a.reverse.each do |dont_use_this_var, spec|
            next if spec.name == 'fundler' && spec.version.to_s != VERSION
            have_fundler = true if spec.name == 'fundler'
            spec.source = self
            idx << spec
          end

          # Always have fundler locally
          unless have_fundler
           # We're running fundler directly from the source
           # so, let's create a fake gemspec for it (it's a path)
           # gemspec
           fundler = Gem::Specification.new do |s|
             s.name     = 'fundler'
             s.version  = VERSION
      ...
      def cached_specs
        @cached_specs ||= begin
          ...
          path = Fundler.app_cache
          Dir["#{path}/*.gem"].each do |gemfile|
            next if gemfile =~ /fundler\-[\d\.]+?\.gem/
      ...
      def remote_specs
        @remote_specs ||= begin
          ...
          remotes.each do |uri|
            Fundler.ui.info "Fetching source index for #{uri}"
            Gem.sources = ["#{uri}"]
            fetch_all_remote_specs do |n,v|
              v.each do |name, version, platform|
                next if name == 'fundler'
      ...
      def fetch_all_remote_specs(&blk)
        begin
          ...
          begin
            ...
            Fundler.ui.warn "Could not fetch prerelease specs from #{self}"
          end
        rescue Gem::RemoteFetcher::FetchError
          Fundler.ui.warn "Could not reach #{self}"
        end
      end
      ...
      def download_gem_from_uri(spec, uri)
        ...
        download_path = Fundler.requires_sudo? ? Fundler.tmp : Gem.dir
        ...
        if Fundler.requires_sudo?
          ...
          sudo "mv #{Fundler.tmp}/cache/#{spec.full_name}.gem #{gem_path}"
        end
    ...
    class Path
      ...
      def initialize(options)
        ...
        if options["path"]
          ...
          @path = @path.expand_path(Fundler.root) unless @path.relative?
        end
      ...
      def eql?(o)
        o.instance_of?(Path) &&
        path.expand_path(Fundler.root) == o.path.expand_path(Fundler.root) &&
        name == o.name &&
        version == o.version
      end
      ...
      def load_spec_files
        ...
        expanded_path = path.expand_path(Fundler.root)

        if File.directory?(expanded_path)
          Dir["#{expanded_path}/#{@glob}"].each do |file|
            spec = Fundler.load_gemspec(file)
            if spec
      ...
      class Installer < Gem::Installer
        def initialize(spec, options = {})
          @spec              = spec
          @bin_dir           = Fundler.requires_sudo? ? "#{Fundler.tmp}/bin" : "#{Gem.dir}/bin"
        ...
        def generate_bin
          ...
          if Fundler.requires_sudo?
            FileUtils.mkdir_p("#{Fundler.tmp}/bin") unless File.exist?("#{Fundler.tmp}/bin")
          end
          super
          if Fundler.requires_sudo?
            Fundler.mkdir_p "#{Gem.dir}/bin"
            spec.executables.each do |exe|
              Fundler.sudo "cp -R #{Fundler.tmp}/bin/#{exe} #{Gem.dir}/bin/"
            end
          end
        end
      end
      ...
      def install(spec)
        Fundler.ui.info "Using #{spec.name} (#{spec.version}) from #{to_s} "
      ...
      def cache(spec)
        unless path.expand_path(Fundler.root).to_s.index(Fundler.root.to_s) == 0
          Fundler.ui.warn "  * #{spec.name} at `#{path}` will not be cached."
        end
      end
    ...
    private
      ...
      def relative_path
        if path.to_s.include?(Fundler.root.to_s)
          return path.relative_path_from(Fundler.root)
        end
      ...
      def generate_bin(spec)
        ...
        Fundler.ui.warn "\n#{spec.name} at #{spec.full_gem_path} did not have a valid gemspec.\n" \
                        "This prevents fundler from installing bins or native extensions, but " \
                        "that may not affect its functionality."

        if !spec.extensions.empty? && !spec.email.empty?
          Fundler.ui.warn "If you need to use this package without installing it from a gem " \
                          "repository, please contact #{spec.email} and ask them " \
                          "to modify their .gemspec so it can work with `gem build`."
        end

        Fundler.ui.warn "The validation message from Rubygems was:\n  #{e.message}"
      ensure
    ...
    class Git < Path
      ...
      def install(spec)
        ...
        unless @installed
          Fundler.ui.debug "  * Checking out revision: #{ref}"
    ...
    private
      ...
      def cache
        if cached?
          ...
          Fundler.ui.info "Updating #{uri}"
          in_cache do
            git %|fetch --force --quiet --tags "#{uri}" refs/heads/*:refs/heads/*|
          end
        else
          Fundler.ui.info "Fetching #{uri}"
